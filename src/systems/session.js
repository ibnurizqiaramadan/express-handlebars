...
class Session {
    ...
    setSession(session) {
        const sessionToken = jwt.sign(session, process.env.APP_SESSION_SECRET, {
            expiresIn: parseInt(process.env.APP_SESSION_EXPIRE),
        });
        this.res.cookie(sessionCookieName, sessionToken, {
            ...
            path: '/',
            expiresIn: new Date(
                Date.now() + parseInt(process.env.APP_SESSION_EXPIRE)
            ),
        });
    }
